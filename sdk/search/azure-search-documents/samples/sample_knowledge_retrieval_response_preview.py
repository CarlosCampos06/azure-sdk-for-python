...
DESCRIPTION:
    ...
    Demonstrates what is new in the knowledge base retrieve response:
      * max_output_documents to bound the number of references returned.
      * include_activity to surface the activity records, including the
        model_name on model activity records.
      * Purview sensitivity labels exposed on references when available.
      * The two preview output modes: extractive_data and answer_synthesis.
...
def retrieve_with_preview_response_fields():
    # [START retrieve_with_preview_response_fields]
    from azure.core.credentials import AzureKeyCredential
    from azure.search.documents import ApiVersion
    ...
    # [END retrieve_with_preview_response_fields]


def retrieve_with_answer_synthesis():
    ...
    # [START retrieve_with_answer_synthesis]
    from azure.core.credentials import AzureKeyCredential
    from azure.search.documents import ApiVersion
    from azure.search.documents.indexes import SearchIndexClient
    from azure.search.documents.indexes.models import (
    ...
    # [END retrieve_with_answer_synthesis]
...
def cleanup():
    ...
    try:
        ...
        print(f"Deleted: {knowledge_base_name}")
    except ResourceNotFoundError:
        pass
    try:
        index_client.delete_knowledge_source(knowledge_source_name)
        print(f"Deleted: {knowledge_source_name}")
    except ResourceNotFoundError:
        pass
...
if __name__ == "__main__":
    try:
        setup()
        retrieve_with_preview_response_fields()
        retrieve_with_answer_synthesis()
    finally:
        cleanup()